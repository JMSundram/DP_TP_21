...
def run_model(par):
    ...
    for z in [0, 1]:
        ...
        for t in reversed(range(par.T-1)):
            for i_h, h in enumerate(par.grid_h):
                if t >= par.TR and i_h != 0: # After retirement, solution is independent of z and h [1]
    ...
    #@jit(double(par_.class_type.instance_type, sol_.class_type.instance_type, sim_.class_type.instance_type), nopython = True, parallel = True)
    @njit(parallel=True)
